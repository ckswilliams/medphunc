...
def dcmshow(d, ax=None, window_override=None, image_number=None):
    ...
    if window_override is not None:
        tim = apply_window(d.pixel_array, window_override)
...
def select_roi_dcm(d, window_override=None):
    window = d.WindowWidth
    level = d.WindowCenter
    if window_override is not None:
        tim = apply_window(d.pixel_array, window_override, unit_range=True)
    else:
        tim = apply_window(d.pixel_array, (window,level), unit_range=True)
    return cv2.selectROI('image_window', tim)


def plot_cv_roi(obj: pydicom.Dataset | np.typing.ArrayLike, roi: tuple, window_level: tuple|str=None):
    """
    Plot a ROI generated by the opencv selectROI function on top of a dicom image or image array.
    

    Parameters
    ----------
    obj : pydicom.Dataset | np.typing.ArrayLike
        Either a pydicom dataset or a numpy array.
    roi : tuple
        The ROI definitions, defined as (x_left,y_top,width, height)
    window_level : tuple|str, optional
        Choose a window/level. If a dicom file is provided, this overrides the included DICOM tag. The default is None.

    Raises
    ------
    ValueError
        If an object other than a numpy array or pydicom dataset is provided, raise a value error.
    ...
    if isinstance(obj, pydicom.Dataset):
        dcmshow(obj, ax=ax, window_override=window_level)
    elif isinstance(obj, np.ndarray):
        tim = apply_window(window_level)
        ax.imshow(tim, cmap='bone')
        ax.axis('off')
    else:
        raise ValueError('Only pydicom and np arrays permitted')
